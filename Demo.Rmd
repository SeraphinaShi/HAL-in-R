...
With the goal of demonstrating how to use HAL, we will only use observations that had the 'Control' or 'Handwashing' treatments. Then we will set the treatment variable (<span class="grey-highlight">tr</span>) to be 1 if being treated by "Handwashing."
```{r}
washb_data_demo <- washb_data[washb_data$tr %in% c("Control", "Handwashing"), ]
washb_data_demo$tr = as.numeric(washb_data_demo$tr == "Handwashing")
```
...
x_names = c("tr", "month", "aged", "sex", "momage", "momheight", "hfiacat", "Nlt18", "Ncomp")
X <- washb_data_demo %>% 
